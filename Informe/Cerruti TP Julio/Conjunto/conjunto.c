...
*/
int conjunto_elemento_valores_interseca(int izqElem1, int derElem1,
                                        int izqElem2, int derElem2) {
...
int conjunto_comparar_elementos_by_extremo_izquierdo(void *elem1, void *elem2) {
  return ((ElementoConjunto *) elem1)->extremoIzq -
      ((ElementoConjunto *) elem2)->extremoIzq;
}
...
*/
void conjunto_extension_elemento(ElementoConjunto * elemento,
                                 ElementoConjunto * extension) {
  elemento->extremoIzq =
      extension->extremoIzq <
      elemento->extremoIzq ? extension->extremoIzq : elemento->extremoIzq;
  elemento->extremoDer =
      extension->extremoDer >
      elemento->extremoDer ? extension->extremoDer : elemento->extremoDer;
}
...
int conjunto_compara_nombre(void *conjunto1, void *conjunto2) {
  return !strcmp(((Conjunto *) conjunto1)->nombre,
                 ((Conjunto *) conjunto2)->nombre);
}
...
Conjunto *conjunto_normalize(Conjunto * conjunto) {
  ...
  while (aux != conjunto->conjunto->prev) {     // mientras no este en el ultimo nodo
    if (extremo_menos_uno(((ElementoConjunto *) aux->next->data)->extremoIzq) ==
        ((ElementoConjunto *) aux->data)->extremoDer
        || ((ElementoConjunto *) aux->next->data)->extremoIzq ==
        ((ElementoConjunto *) aux->data)->extremoDer) {
...
void conjunto_imprimir(Conjunto conjunto) {
  ...
  if (conjunto.conjunto) {
    do {
      if (((ElementoConjunto *) aux->data)->extremoIzq ==
          ((ElementoConjunto *) aux->data)->extremoDer)
        printf("%d", ((ElementoConjunto *) aux->data)->extremoIzq);
      else
        printf("%d:%d", ((ElementoConjunto *) aux->data)->extremoIzq,
               ((ElementoConjunto *) aux->data)->extremoDer);
      aux = aux->next;
...
Conjunto *conjunto_union(Conjunto conjunto1, Conjunto conjunto2,
  ...
  while (aux1 && aux2) {        // mientras ambos tengan elementos sin unir
    if (!elemAux) {             // si no existe el nodo a agregar
      ...
      if (((ElementoConjunto *) aux1->data)->extremoIzq <
          ((ElementoConjunto *) aux2->data)->extremoIzq) {
      ...
      if (conjunto_elemento_valores_interseca
          (extremo_menos_uno(elemAux->extremoIzq),
           extremo_mas_uno(elemAux->extremoDer),
           ((ElementoConjunto *) aux1->data)->extremoIzq,
           ((ElementoConjunto *) aux1->data)->extremoDer)
          ||
          conjunto_elemento_valores_interseca(extremo_menos_uno
                                              (elemAux->extremoIzq),
                                              extremo_mas_uno(elemAux->
                                                              extremoDer),
                                              ((ElementoConjunto *) aux2->
                                               data)->extremoIzq,
                                              ((ElementoConjunto *) aux2->
                                               data)->extremoDer)) {
        if (conjunto_elemento_valores_interseca
        ...
        if (conjunto_elemento_valores_interseca
            (extremo_menos_uno(elemAux->extremoIzq),
             extremo_mas_uno(elemAux->extremoDer),
             ((ElementoConjunto *) aux2->data)->extremoIzq,
             ((ElementoConjunto *) aux2->data)->extremoDer)) {
      ...
      if (!elemAux) {
        ...
        if (conjunto_elemento_valores_interseca
          ...
          newConjunto->conjunto =
              glist_insert_last_position(newConjunto->conjunto, elemAux);
...
Conjunto *conjunto_interseccion(Conjunto conjunto1, Conjunto conjunto2,
                                char *nombreConjunto) {
  GList aux1 = conjunto1.conjunto, aux2 = conjunto2.conjunto;
  ...
  while (aux1 && aux2) {        // mientras haya elementos en ambos conjuntos
    if (conjunto_elemento_valores_interseca
      ...
      elemAux->extremoIzq =
          ...
          extremoIzq ? ((ElementoConjunto *) aux1->data)->
          extremoIzq : ((ElementoConjunto *) aux2->data)->extremoIzq;
      elemAux->extremoDer =
          ((ElementoConjunto *) aux1->data)->extremoDer <
          ((ElementoConjunto *) aux2->data)->
...
Conjunto *conjunto_complemento(Conjunto conjunto, char *nombreConjunto) {
  ...
  while (aux != conjunto.conjunto) {
    elemAux = malloc(sizeof(ElementoConjunto));
    elemAux->extremoIzq =
        ((ElementoConjunto *) aux->prev->data)->extremoDer + 1;
    elemAux->extremoDer = ((ElementoConjunto *) aux->data)->extremoIzq - 1;
    newConjunto->conjunto =
        glist_insert_last_position(newConjunto->conjunto, elemAux);
