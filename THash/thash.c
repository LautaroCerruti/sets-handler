...
SLList* tabla_agregar_elemento(SLList* tabla, void* dato, FuncionHasheo hasheo, Compare comparar, Destroy destruir) {
    int valhasheo = hasheo(dato);
    tabla[valhasheo] = sllist_insert_with_replace(tabla[valhasheo], dato, comparar, destruir);
    return tabla;
}

void* tabla_buscar_elemento(SLList* tabla, void* dato, FuncionHasheo hasheo, Compare comparar, Destroy destruir) {
    int valhasheo = hasheo(dato);
