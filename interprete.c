...
void imprimir(THash tabla, char *string) {
  if (check_alphanumeric(string)) {     // Si no es valido el nombre del cojunto
    ...
    Conjunto *conjunto =
        tabla_buscar_elemento(tabla, string, hasheo_string,
...
*/
int inserta_conjunto_comprension(THash tabla, char *string,
  ...
  conjunto =
      ...
      conjunto_create_extension(nombre_conjunto, numero1, numero2);
  tabla =
...
/*
  ...
  corresponde
*/
void realizar_operacion(THash tabla, char operacion,
                        char *nombre_primer_conjunto,
                        char *nombre_segundo_conjunto,
                        char *nombre_tercer_conjunto) {
  Conjunto *primerConjuntoOperacion =
      tabla_buscar_elemento(tabla, nombre_segundo_conjunto, hasheo_string,
                            comparar_conjunto_string),
  ...
  if ((primerConjuntoOperacion && operacion == '~')
    ...
    if (operacion == '~')
      conjuntoResultado =
          conjunto_complemento(*primerConjuntoOperacion,
                               nombre_primer_conjunto);
    if (operacion == '|')
      conjuntoResultado =
          conjunto_union(*primerConjuntoOperacion, *segundoConjuntoOperacion,
                         nombre_primer_conjunto);
    if (operacion == '-')
      conjuntoResultado =
          conjunto_resta(*primerConjuntoOperacion, *segundoConjuntoOperacion,
                         nombre_primer_conjunto);
    if (operacion == '&')
      conjuntoResultado =
          conjunto_interseccion(*primerConjuntoOperacion,
                                *segundoConjuntoOperacion,
                                nombre_primer_conjunto);
    // realiza la operacion correspondiente y agrega el conjunto a la tabla
    tabla =
        tabla_agregar_elemento(tabla, conjuntoResultado, conjunto_hash,
                               conjunto_compara_nombre,
                               conjunto_destroy_conjunto);
...
int main() {
  char buffer[MAX_LINEA], aux[MAX_NOMBRE_CONJUNTO], *bufferAux,
      *nombre_primer_conjunto = NULL, *nombre_segundo_conjunto =
      NULL, *nombre_tercer_conjunto = NULL, operacion;
  int i = 0, error = 0;
  buffer[0] = '\0';
      ...
      if (!error) {
        ...
        if (buffer[i] == '{') { // si es una creacion de conjunto
          ...
          if (isalpha(buffer[i]) && buffer[i] != '-') {
            error =
                inserta_conjunto_comprension(tabla, buffer + i,
                                             nombre_primer_conjunto);
          } else if (isdigit(buffer[i]) || buffer[i] == '-') {
            error =
                inserta_conjunto_extension(tabla, buffer + i,
                                           nombre_primer_conjunto);
          } else
            error = 1;
          ...
          if (*bufferAux == ' ')
            ++bufferAux;
          for (i = 0; i < MAX_NOMBRE_CONJUNTO && *bufferAux != '\0' && !error;
               ++bufferAux) {
          ...
          if (*bufferAux == '\0' && i > 0 && !error) {
            if (aux[i - 1] == ' ')
              ...
              aux[i] = '\0';
            if (check_alphanumeric(aux)) {
              nombre_segundo_conjunto =
                  malloc(sizeof(char) * (strlen(aux) + 1));
            ...
            error = 1;
          if (!error)
            realizar_operacion(tabla, operacion, nombre_primer_conjunto,
                               nombre_segundo_conjunto, nombre_tercer_conjunto);
        } else if (!isalnum(buffer[i]))
          error = 1;
        else {
          bufferAux = buffer + i;
          for (i = 0;
               i < MAX_NOMBRE_CONJUNTO && *bufferAux != '-' && *bufferAux != '&'
               && *bufferAux != '|' && *bufferAux != '\0' && !error;
               ++bufferAux) {
            aux[i] = *bufferAux;        // lee el primer conjunto a operar
            ++i;
          }

          if ((*bufferAux == '-' || *bufferAux == '&' || *bufferAux == '|')
              && i > 0 && !error) {
            operacion = *bufferAux;
            if (aux[i - 1] == ' ')
              aux[i - 1] = '\0';
